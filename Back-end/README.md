# LLM智能出题考试系统 - 后端

基于FastAPI的高性能异步后端服务

## 技术栈

- **框架**: FastAPI 0.115+
- **数据库**: SQLite (异步) / PostgreSQL (可选)
- **ORM**: SQLAlchemy 2.0 (异步模式)
- **认证**: JWT + bcrypt
- **LLM**: 多模型支持 (DeepSeek/Qwen/GLM)
- **Python**: 3.10+

## 快速开始

### 1. 创建虚拟环境

```bash
# Windows
python -m venv venv
venv\Scripts\activate

# Linux/Mac
python3 -m venv venv
source venv/bin/activate
```

### 2. 安装依赖

```bash
pip install -r requirements.txt
```

### 3. 配置环境变量

```bash
# 复制环境变量模板
cp .env.example .env

# 编辑.env文件,配置必要的参数
# 重要: 修改SECRET_KEY和LLM API密钥
```

### 4. 启动开发服务器

```bash
# 方式1: 使用uvicorn
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000

# 方式2: 直接运行main.py
python -m app.main

# 方式3: 使用fastapi cli (需要安装fastapi-cli)
fastapi dev app/main.py
```

### 5. 访问API文档

- Swagger UI: http://localhost:8000/api/docs
- ReDoc: http://localhost:8000/api/redoc
- 健康检查: http://localhost:8000/health

## 项目结构

```
Back-end/
├── app/
│   ├── __init__.py           # 应用包初始化
│   ├── main.py               # FastAPI应用入口
│   ├── config.py             # 配置管理 (Pydantic Settings)
│   ├── api/                  # API路由模块
│   │   ├── __init__.py       # 路由导出
│   │   ├── deps.py           # ✅ 依赖注入 (认证、权限)
│   │   ├── auth.py           # ✅ 认证路由
│   │   ├── llm.py            # ✅ LLM通用路由
│   │   ├── questions.py      # ✅ 题目生成路由
│   │   ├── courses.py        # ✅ 课程管理路由
│   │   └── exams.py          # ✅ 考试管理路由 🆕
│   ├── models/               # SQLAlchemy数据库模型
│   │   ├── __init__.py       # 模型导出
│   │   ├── user.py           # ✅ 用户模型
│   │   ├── course.py         # ✅ 课程/知识点模型
│   │   ├── question.py       # ✅ 题目/试卷模型
│   │   ├── exam.py           # ✅ 考试/答题模型
│   │   └── llm_log.py        # ✅ LLM日志模型
│   ├── schemas/              # Pydantic请求/响应模型
│   │   ├── __init__.py       # Schema导出
│   │   ├── user.py           # ✅ 用户Schema
│   │   ├── question.py       # ✅ 题目Schema
│   │   ├── course.py         # ✅ 课程/知识点Schema
│   │   └── exam.py           # ✅ 考试/答题Schema 🆕
│   ├── services/             # 业务逻辑层
│   │   ├── __init__.py       # 服务导出
│   │   ├── auth.py           # ✅ 认证服务
│   │   ├── llm_service.py    # ✅ LLM统一服务
│   │   ├── generator_service.py   # ✅ 题目生成器
│   │   ├── validator_service.py   # ✅ 规则校验器
│   │   ├── reviewer_service.py    # ✅ AI自审服务
│   │   ├── generation_pipeline.py # ✅ 生成流水线
│   │   ├── course_service.py # ✅ 课程管理服务
│   │   └── exam_service.py   # ✅ 考试管理服务 🆕
│   ├── core/                 # 核心工具
│   │   ├── __init__.py       # 工具导出
│   │   ├── security.py       # ✅ JWT/密码加密
│   │   └── llm/              # ✅ LLM Provider抽象
│   │       ├── __init__.py
│   │       ├── base.py       # ✅ 抽象基类
│   │       ├── deepseek.py   # ✅ DeepSeek Provider
│   │       ├── qwen.py       # ✅ Qwen Provider
│   │       └── glm.py        # ✅ GLM Provider
│   └── db/                   # 数据库相关
│       ├── __init__.py       # 数据库导出
│       ├── base.py           # ✅ Base类/Mixin
│       ├── session.py        # ✅ 数据库会话
│       └── init_db.py        # ✅ 数据库初始化
├── docs/                     # 文档目录
│   └── DATABASE_DESIGN.md    # ✅ 数据库设计文档
├── tests/                    # 测试文件
├── logs/                     # 日志文件 (自动生成)
├── .env.example              # 环境变量模板
├── .env                      # 环境变量 (需创建)
├── requirements.txt          # Python依赖
├── test_auth.py              # 认证测试脚本
└── README.md                 # 本文件
```

> ✅ = 已实现 | 🚧 = 待实现

---

## 核心功能

### 1. 用户认证与授权 ✅

- JWT Token认证 (7天有效期)
- 角色权限控制 (学生/教师/管理员)
- bcrypt密码加密存储
- OAuth2兼容的登录接口

### 2. LLM调用封装层 ✅

多模型Provider抽象架构，支持灵活切换：

```
┌─────────────────────────────────────────────────────────────┐
│                      LLMService                             │
│              (统一接口 + 日志记录 + 错误处理)                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐       │
│   │  DeepSeek   │  │    Qwen     │  │     GLM     │       │
│   │  Provider   │  │  Provider   │  │  Provider   │       │
│   └──────┬──────┘  └──────┬──────┘  └──────┬──────┘       │
│          │                │                │              │
│          ▼                ▼                ▼              │
│   ┌─────────────────────────────────────────────────┐     │
│   │           BaseLLMProvider (抽象基类)             │     │
│   │  - chat(): 普通对话                              │     │
│   │  - chat_stream(): 流式对话                       │     │
│   └─────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────┘
```

**特性**:
- 统一的OpenAI兼容API格式
- 自动日志记录 (token数、耗时、场景)
- 支持流式输出 (SSE)
- 可配置的模型参数 (temperature, max_tokens等)

**配置示例** (`.env`):
```bash
LLM_PROVIDER=deepseek
DEEPSEEK_API_KEY=your-api-key
DEEPSEEK_BASE_URL=https://api.deepseek.com/v1
DEEPSEEK_MODEL=deepseek-chat
```

### 3. AI出题质量控制流水线 ✅

**三阶段流水线架构** - 本项目的核心创新点：

```
┌─────────────────────────────────────────────────────────────────────┐
│                      GenerationPipeline                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   Stage 1              Stage 2              Stage 3                │
│  ┌──────────┐        ┌──────────┐        ┌──────────┐             │
│  │Generator │  ───▶  │Validator │  ───▶  │ Reviewer │             │
│  │ (LLM)    │        │ (规则)   │        │  (LLM)   │             │
│  └──────────┘        └────┬─────┘        └────┬─────┘             │
│                           │                    │                   │
│                      ┌────▼────┐          ┌────▼────┐             │
│                      │ 格式错误 │          │ 有问题  │             │
│                      │ REJECTED │          │         │             │
│                      └─────────┘          ┌────▼────┐             │
│                                           │尝试修复 │             │
│                                           └────┬────┘             │
│                                                │                   │
│                                       ┌────────┴────────┐         │
│                                       ▼                 ▼         │
│                                  ┌─────────┐      ┌───────────┐   │
│                                  │APPROVED │      │NEEDS_REVIEW│   │
│                                  │ (通过)  │      │(人工审核) │   │
│                                  └─────────┘      └───────────┘   │
└─────────────────────────────────────────────────────────────────────┘
```

#### Stage 1: Generator (生成器)
- 使用LLM根据要求生成题目
- 支持多种题型：单选、多选、填空、简答
- 可配置难度、知识点、语言等参数

#### Stage 2: Validator (规则校验器)
代码实现的规则校验：
- ✅ JSON格式校验
- ✅ 单选题：答案在选项内且唯一
- ✅ 多选题：答案集合非空且都在选项内
- ✅ 填空题：空数量匹配答案数量
- ✅ 简答题：提供评分要点
- ✅ 去重检测：题干哈希比对

#### Stage 3: Reviewer (AI自审)
使用LLM审核题目质量：
- 事实正确性检查
- 答案唯一性/歧义检测
- 表述清晰性审核
- 难度匹配度评估
- 自动修复一次机会

**输出分类**:
| 状态 | 说明 |
|------|------|
| `approved` | 通过所有检查，可直接使用 |
| `needs_review` | 需要人工审核确认 |
| `rejected` | 格式错误，无法使用 |

### 4. 课程与知识点管理 ✅
- 课程的增删改查
- 知识点无限层级树形结构管理
- 级联删除保护 (删除课程自动清理知识点)

### 5. 题目管理 🚧
- 多题型支持 (单选/多选/填空/简答)
- 知识点关联
- 难度分级 (1-5级)

### 5. 考试系统 ✅
- 考试创建、编辑、删除
- 考试发布与关闭
- 学生开始、提交考试
- 客观题自动评分

### 6. 学习分析 🚧
- 错题本
- 薄弱点统计
- 学习建议

---

## API 端点

> 完整的数据库设计文档请参阅: [docs/DATABASE_DESIGN.md](../docs/DATABASE_DESIGN.md)

### 认证模块 (`/api/auth`) ✅

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/auth/register` | 用户注册 | 否 |
| POST | `/api/auth/login` | 用户登录 (JSON) | 否 |
| POST | `/api/auth/login/form` | 用户登录 (OAuth2表单) | 否 |
| GET | `/api/auth/me` | 获取当前用户信息 | 是 |
| PUT | `/api/auth/me` | 更新用户资料 | 是 |

<details>
<summary>📝 认证API示例</summary>

**注册用户**
```bash
curl -X POST http://localhost:8000/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email": "teacher@example.com",
    "name": "张老师",
    "password": "password123",
    "role": "teacher"
  }'
```

**登录获取Token**
```bash
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "teacher@example.com",
    "password": "password123"
  }'
```

**响应示例**
```json
{
  "access_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "token_type": "bearer",
  "user": {
    "id": 1,
    "email": "teacher@example.com",
    "name": "张老师",
    "role": "teacher",
    "is_active": true
  }
}
```

</details>

### LLM通用模块 (`/api/llm`) ✅

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/llm/chat` | 发送对话请求 | 是 |
| POST | `/api/llm/chat/stream` | 流式对话 (SSE) | 是 |
| POST | `/api/llm/simple` | 简单文本请求 | 是 |
| POST | `/api/llm/simple/stream` | 简单文本流式 | 是 |
| GET | `/api/llm/providers` | 获取可用Provider列表 | 是 |

<details>
<summary>📝 LLM API示例</summary>

**简单对话**
```bash
curl -X POST http://localhost:8000/api/llm/simple \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "content": "什么是Python的装饰器？",
    "system_prompt": "你是一个编程专家"
  }'
```

**流式对话**
```bash
curl -X POST http://localhost:8000/api/llm/chat/stream \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [
      {"role": "user", "content": "请解释什么是闭包"}
    ]
  }'
```

</details>

### 题目生成模块 (`/api/questions`) ✅

| 方法 | 路径 | 说明 | 认证 | 角色 |
|------|------|------|------|------|
| POST | `/api/questions/generate` | 完整流水线生成 | 是 | 教师 |
| POST | `/api/questions/generate/quick` | 快速生成 (跳过AI审核) | 是 | 教师 |
| POST | `/api/questions/generate/stream` | 流式生成 (SSE) | 是 | 教师 |
| GET | `/api/questions/types` | 获取支持的题型 | 否 | - |
| GET | `/api/questions/difficulties` | 获取难度等级 | 否 | - |

<details>
<summary>📝 题目生成API示例</summary>

**生成题目 (完整流水线)**
```bash
curl -X POST http://localhost:8000/api/questions/generate \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "course_name": "Python编程",
    "knowledge_point": "列表操作",
    "question_type": "single",
    "difficulty": 3,
    "count": 3,
    "language": "zh"
  }'
```

**响应示例**
```json
{
  "summary": {
    "total": 3,
    "approved": 2,
    "needs_review": 1,
    "rejected": 0,
    "success_rate": 66.7
  },
  "approved_questions": [
    {
      "question": {
        "type": "single",
        "stem": "下列哪个方法可以向Python列表末尾添加元素？",
        "options": {
          "A": "add()",
          "B": "append()",
          "C": "insert()",
          "D": "extend()"
        },
        "answer": "B",
        "explanation": "append()方法用于在列表末尾添加单个元素...",
        "difficulty": 3,
        "knowledge_point": "列表操作"
      },
      "review_comment": "题目正确，无歧义"
    }
  ],
  "needs_review_questions": [...],
  "rejected_questions": []
}
```

**快速生成 (跳过AI审核)**
```bash
curl -X POST http://localhost:8000/api/questions/generate/quick \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "course_name": "数据结构",
    "question_type": "multiple",
    "difficulty": 4,
    "count": 5
  }'
```

**支持的题型**
```bash
curl http://localhost:8000/api/questions/types
```
```json
{
  "types": [
    {"value": "single", "label": "单选题", "description": "四选一，只有一个正确答案"},
    {"value": "multiple", "label": "多选题", "description": "四选多，有2-3个正确答案"},
    {"value": "blank", "label": "填空题", "description": "在题干空白处填入正确答案"},
    {"value": "short", "label": "简答题", "description": "需要详细回答的主观题"}
  ]
}
```

</details>

### 待实现的 API 🚧

| 模块 | 前缀 | 状态 |
|------|------|------|
| 课程管理 | `/api/courses` | ✅ 已实现 |
| 考试管理 | `/api/exams` | ✅ 已实现 |
| 试卷管理 | `/api/papers` | 🚧 待实现 |
| AI批改 | `/api/grading` | 🚧 待实现 |
| 统计分析 | `/api/analytics` | 🚧 待实现 |

---

## 开发规范

### 代码风格
- 使用Black格式化代码
- 使用Ruff进行代码检查
- 遵循PEP 8规范

### 类型注解
所有函数必须添加类型注解:
```python
async def get_user(user_id: int) -> User:
    ...
```

### 异步优先
优先使用async/await:
```python
@app.get("/users/{user_id}")
async def get_user(user_id: int):
    user = await db.get_user(user_id)
    return user
```

### 错误处理
使用FastAPI的HTTPException:
```python
from fastapi import HTTPException

if not user:
    raise HTTPException(status_code=404, detail="用户不存在")
```

## 环境变量说明

| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| SECRET_KEY | JWT密钥 | (必须修改) |
| DATABASE_URL | 数据库连接 | sqlite+aiosqlite:///./app.db |
| LLM_PROVIDER | LLM提供商 | deepseek |
| DEEPSEEK_API_KEY | DeepSeek API密钥 | (需配置) |
| DEEPSEEK_BASE_URL | DeepSeek API地址 | https://api.deepseek.com/v1 |
| DEEPSEEK_MODEL | DeepSeek模型名 | deepseek-chat |
| LLM_TEMPERATURE | 生成温度 | 0.7 |
| LLM_MAX_TOKENS | 最大token数 | 4000 |
| LLM_TIMEOUT | 超时时间(秒) | 60 |
| CORS_ORIGINS | 允许的前端域名 | ["http://localhost:5173"] |

## 常用命令

```bash
# 格式化代码
black app/

# 代码检查
ruff check app/

# 运行测试
pytest

# 查看依赖树
pip list

# 导出依赖
pip freeze > requirements.txt
```

## 数据库迁移

(待实现 Alembic)

## 部署

(待补充 Docker部署说明)

## 许可证

MIT License

---

**开发状态**: 🚧 开发中
**版本**: v1.1.0
**最后更新**: 2026-01-09
