...
### 第6阶段: 前端改造 (1-2周)
- [x] 移除前端LLM直连 (API密钥不再存储在前端)
- [x] 对接后端API (LLM客户端层改造完成)
- [x] 添加Vite开发代理配置
- [ ] **添加登录/注册页面** ⬅️ 当前任务
- [ ] 添加认证状态管理 (存储JWT Token)
- [ ] 添加路由保护 (未登录跳转登录页)
- [ ] 添加课程/知识点管理页
...
- ✅ 后端: 核心功能已实现 (认证 + LLM封装 + 题目生成流水线)
- ✅ 前端LLM层: 已改造为调用后端API (不再直连LLM厂商)
- 🚧 前端认证: **需要添加登录/注册页面**
- 📋 计划: 完成前端认证系统后，可进行完整联调
...
src/pages/
├── LoginPage.tsx       # 登录页面
└── RegisterPage.tsx    # 注册页面 (可选，也可合并)
```

#### 2. 认证状态管理
```
src/stores/
└── useAuthStore.ts     # 认证状态 (user, token, isLoggedIn)
```

#### 3. API服务层
```
src/services/
└── authService.ts      # 封装后端认证API调用
```
