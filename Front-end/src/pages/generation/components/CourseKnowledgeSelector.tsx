...
import { ChevronDown, ChevronRight, BookOpen, Lightbulb, X } from 'lucide-react';
 ...
 * 课程与知识点选择器
 */
export const CourseKnowledgeSelector: React.FC<CourseKnowledgeSelectorProps> = ({
    courseId,
    knowledgePointId,
    onCourseChange,
    onKnowledgePointChange,
}) => {
    const { courses, currentCourse, fetchCourses, fetchCourse, clearCurrentCourse } =
        useCourseStore();
    const [isExpanded, setIsExpanded] = useState(true);
    ...
    const getSelectedPointPath = (): string => {
        ...
        const findPath = (
            ...
            for (const point of points) {
                ...
                if (point.children) {
                    const result = findPath(point.children, targetId, [...path, point.name]);
        ...
        const path = findPath(currentCourse.knowledge_points || [], knowledgePointId, []);
