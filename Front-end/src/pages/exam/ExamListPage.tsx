...
const getExamAvailability = (
  ...
  if (startTime && !Number.isNaN(startTime) && now < startTime) {
    return { canStart: false, label: '未开始' };
  }
  if (endTime && !Number.isNaN(endTime) && now > endTime) {
    return { canStart: false, label: '已结束' };
  }
  if (exam.can_start === false) {
    return { canStart: false, label: '不可进入' };
  }
...
interface ExamCardProps {
  ...
  availability: { canStart: boolean; label?: string };
}
